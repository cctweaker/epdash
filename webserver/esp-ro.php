...
$zile_scurt = array('Du', 'Lu', 'Ma', 'Mi', 'Jo', 'Vi', 'Sa');
...
imagestring($im, 3, 425, 16, $zile[date("w", $weather['dt'])] . ' ' . date("d", $weather['dt']) . ' ' . $luni[date("n", $weather['dt'])] . ' la ' . date("H:i", $weather['dt']), $black);
...
if (isset($weather['clouds']['all'])) {
    if ($weather['clouds']['all'] > 0) {
        imagestring($im, 5, 1, $y + 80, number_format((float) $weather['clouds']['all'], 0) . '% nori', $black);
    }
}
if (isset($weather['rain']['3h'])) {
    if ($weather['rain']['3h'] > 0) {
        imagestring($im, 5, 1, $y + 96, number_format((float) $weather['rain']['3h'], 2) . ' ploaie', $black);
    }
} elseif (isset($weather['snow']['3h'])) {
    if ($weather['snow']['3h'] > 0.1) {
        imagestring($im, 5, 1, $y + 96, number_format((float) $weather['snow']['3h'], 2) . ' zapada', $black);
    }
}
...
for ($i = 0; $i < $col; $i++) {
    ...
    imagestring($im, $font, $x * $i, $y1 + 50, '    ' . $zile_scurt[date("w", $forecast['list'][$i]['dt'])] . ' ' . date("H", $forecast['list'][$i]['dt']) . 'h', $black);
    ...
    if (isset($forecast['list'][$i]['clouds']['all']))
        if ($forecast['list'][$i]['clouds']['all'] > 0)
            imagestring($im, $font, $x * $i, $y1 + 50 + 4 * $font_h, '    ' . number_format((float) $forecast['list'][$i]['clouds']['all'], 0) . '% N', $black);
    if (isset($forecast['list'][$i]['rain']['3h']))
        if ($forecast['list'][$i]['rain']['3h'] > 0)
            imagestring($im, $font, $x * $i, $y1 + 50 + 5 * $font_h, '    ' . number_format((float) $forecast['list'][$i]['rain']['3h'], 2) . ' P', $black);
    if (isset($forecast['list'][$i]['snow']['3h']))
        if ($forecast['list'][$i]['snow']['3h'] > 0.1)
            imagestring($im, $font, $x * $i, $y1 + 50 + 5 * $font_h, '    ' . number_format((float) $forecast['list'][$i]['snow']['3h'], 2) . ' Z', $black);


    imagecopyresampled($im, imagecreatefrompng('meteo/' . $forecast['list'][$i + 8]['weather'][0]['icon'] . '.png'), $x * $i + 15, $y2, 0, 0, 50, 50, 256, 256);
    imagestring($im, $font, $x * $i, $y2 + 50, '    ' . $zile_scurt[date("w", $forecast['list'][$i + $col]['dt'])] . ' ' . date("H", $forecast['list'][$i + $col]['dt']) . 'h', $black);
    ...
    if (isset($forecast['list'][$i + $col]['clouds']['all']))
        if ($forecast['list'][$i + $col]['clouds']['all'] > 0)
            imagestring($im, $font, $x * $i, $y2 + 50 + 4 * $font_h, '    ' . number_format((float) $forecast['list'][$i + $col]['clouds']['all'], 0) . '% N', $black);
    if (isset($forecast['list'][$i + $col]['rain']['3h']))
        if ($forecast['list'][$i + $col]['rain']['3h'] > 0)
            imagestring($im, $font, $x * $i, $y2 + 50 + 5 * $font_h, '    ' . number_format((float) $forecast['list'][$i + $col]['rain']['3h'], 2) . ' P', $black);
    if (isset($forecast['list'][$i + $col]['snow']['3h']))
        if ($forecast['list'][$i + $col]['snow']['3h'] > 0.1)
            imagestring($im, $font, $x * $i, $y2 + 50 + 5 * $font_h, '    ' . number_format((float) $forecast['list'][$i + $col]['snow']['3h'], 2) . ' Z', $black);
}
